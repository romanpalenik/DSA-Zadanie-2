 ...
 * 				  for keeping the tree balanced an extra color field is introduced
 *				  to each node.
 ...
 *				  2. ROOT is BLACK.
 *				  3. Leaves of this tree are null nodes. Here null is represented by
 *				  	 a special node NILL. Each NILL nodes are BLACK. So each leave
 * 				 	 is BLACK.
 ...
 *				  5. Each simple path taken from a node to descendent leaf has same
 *					 number of black height. That means each path contains same
